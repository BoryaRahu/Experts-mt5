...
void OnTick(){
 ...
 double _HighBig[];
   ...
   if(totalBars != bars){
      ...
      if(buyPos <= 0//&& _HighBig[2]> _HighBig[1]
      ){
      ...
      if(sellPos <= 0//&& _HighBig[2] <_HighBig[1]
       ){
         double low = findLow();
         if(low > 0){
            executeSell(low);
...
//   trade.BuyStop(lots,entry,_Symbol,sl,tp,ORDER_TIME_SPECIFIED,expiration);
    trade.BuyStop(lots,entry,_Symbol,sl,tp,ORDER_TIME_SPECIFIED,expiration,"BuyStop EuroUsdM5");
...
void executeSell(double entry){
  ...
  // trade.SellStop(lots,entry,_Symbol,sl,tp,ORDER_TIME_SPECIFIED,expiration);
    trade.SellStop(lots,entry,_Symbol,sl,tp,ORDER_TIME_SPECIFIED,expiration,"SellStop EuroUsdM5");
