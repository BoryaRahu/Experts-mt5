...
#define __MT4ORDERS__ "2020.12.06"
#define MT4ORDERS_SLTP_OLD // Включение старого механизма определения SL/TP закрытых позиций через OrderClose
...
//        if (MT4ORDERS::Order.Comment == "") // Комментарий закрытой позиции всегда должен быть равен комментарию открывающей сделки.
        ...
        if (Res) // OrderTicket может не быть в истории, но может оказаться среди еще живых. Возможно, резонно оттуда выудить нужную инфу.
        {
    ...
    if (OrderTicket)
    {
      ...
      if ((ENUM_ORDER_TYPE)::HistoryOrderGetInteger(OrderTicket, ORDER_TYPE) == ORDER_TYPE_CLOSE_BY)
      {
        ...
        if (PosTicketBy == PosTicket) // CloseBy-Slave не должен влиять на торговый оборот.
        {
        ...
        else // CloseBy-Master должен получить комиссию от CloseBy-Slave.
        {
...
*/
    ...
    bool Res = !(::OrderGetInteger(ORDER_POSITION_ID) &&
                 (Market = (::OrderGetInteger(ORDER_TYPE) <= ORDER_TYPE_SELL))); // Partial-отложенник имеет не нулевой PositionID.

    // Если живой Маркет-ордер смог частично исполниться, то он не будет виден с таким условием, хотя должен.
    // https://www.mql5.com/ru/forum/38456/page148#comment_17910929
    if (Res && Market)
    {
      const long Ticket = ::PositionGetInteger(POSITION_TICKET);

      if (_B2(::PositionSelectByTicket(::OrderGetInteger(ORDER_TICKET)))) // Ордер и его позиция могут быть одновременно - данное условие поможет только на Hedge-счетах
      {
